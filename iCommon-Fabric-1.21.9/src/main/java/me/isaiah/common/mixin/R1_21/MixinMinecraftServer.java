...
public class MixinMinecraftServer implements IMixinMinecraftServer {
    ...
    private static NoiseChunkGenerator createGenerator(DynamicRegistryManager registryManager, long seed, RegistryKey<ChunkGeneratorSettings> settings, boolean flag) {
		...
		Registry<Biome> iregistry = registryManager.getOrThrow(RegistryKeys.BIOME);
		
		//RegistryKeys.noise
		
        Registry<StructureSet> iregistry1 = registryManager.getOrThrow(RegistryKeys.STRUCTURE_SET);
        Registry<ChunkGeneratorSettings> iregistry2 = registryManager.getOrThrow(RegistryKeys.CHUNK_GENERATOR_SETTINGS);
        Registry<DoublePerlinNoiseSampler.NoiseParameters> iregistry3 = registryManager.getOrThrow(RegistryKeys.NOISE_PARAMETERS);
	...
	public UUID get_uuid_from_profile(GameProfile profile) {
		return profile.id() != null
				? profile.id()
				: Uuids.getOfflinePlayerUuid(profile.name());
	}
	...
	@Override
	public Text IC$from_json(String jsonMessage) {
		...
		MinecraftServer mc = (MinecraftServer) (Object) this;
		 ...
		 final JsonElement jsonElement = GSON.fromJson(jsonMessage, JsonElement.class);
	        return jsonElement == null ? null : TextCodecs.CODEC.parse(
	            mc.getRegistryManager().getOps(JsonOps.INSTANCE), jsonElement
	...
	public String IC$to_json(Text text) {
		...
		return GSON.toJson(
				TextCodecs.CODEC
	                .encodeStart(mc.getRegistryManager().getOps(JsonOps.INSTANCE), text)
	                .getOrThrow(JsonParseException::new)
	        );
